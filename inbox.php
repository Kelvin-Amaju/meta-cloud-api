...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'reply') {
        ...
        if ($bizId <= 0 || $customerId <= 0 || $message === '') {
            ...
            $business = getBusinessById($bizId);
            ...
            if (!$business) {
                ...
                $response = sendTextMessage($customer['phone'], $message, $business);

                if (!empty($response['success'])) {
                    $wamid = $response['data']['messages'][0]['id'] ?? ('wamid.simulated_' . time());
                    saveOutgoingMessage($customer['phone'], $message, $wamid, 1, $bizId, 'sent', null, 'text');
                    markConversationRead($bizId, $customerId);
                    ...
                    saveOutgoingMessage($customer['phone'], $message, 'wamid.failed_' . time(), 1, $bizId, 'failed', $errorMessage, 'text');
        ...
        if ($bizId > 0 && $customerId > 0) {
            setConversationStatus($bizId, $customerId, $newStatus);
        }
    }
}
...
$result   = getConversations($filters, $page, $perPage);
...
if ($selectedCustomerId > 0) {
    $selectedConversation = getConversationById($selectedCustomerId);
...
if ($selectedConversation) {
    ...
    markConversationRead((int)$selectedConversation['business_id'], (int)$selectedConversation['customer_id']);
}
