...
require_once __DIR__ . '/conversation_functions.inc.php';
 ...
 */
function save_outgoing_message(
    string $toPhone,
    string $message,
    string $wamid,
    int $allowReply = 1,
    ...
    if ($result && $insertId) {
        $customerId = sync_customer_conversation($businessId, $toPhone, 'outbound', $message);
        if ($customerId) {
...
function save_inbound_message(
    ...
    if ($result && $insertId) {
        $customerId = sync_customer_conversation($businessId, $fromNumber, 'inbound', $body ?? ($mediaType ?? $messageType));
        if ($customerId) {
            $upd = $mysqli->prepare("UPDATE business_messages SET customer_id = ? WHERE id = ?");
            if ($upd) {
                $upd->bind_param("ii", $customerId, $insertId);
 ...
 */
function update_message_status_by_wamid(string $wamid, string $status, ?int $businessId = null, ?int $timestamp = null): bool
{
    global $mysqli;
 ...
 */
function get_last_outgoing_message(string $phone): ?array
{
    global $mysqli;
 ...
 */
function get_messages(array $filters = [], int $page = 1, int $perPage = 20): array
{
    global $mysqli;
 ...
 */
function get_message_stats(): array
{
    global $mysqli;
