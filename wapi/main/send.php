...
$selectedBusiness   = $selectedBusinessId > 0 ? get_business($selectedBusinessId) : null;

// Build a per-business template map for the frontend toggle/dropdown
$templatesByBusiness = [];
foreach ($activeBusinesses as $biz) {
    $templatesByBusiness[$biz['id']] = get_templates_for_business($biz['id']);
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $selectedBusiness = get_business($businessId);
    ...
    if (empty($cleanPhone)) {
        ...
        $template = $templateName ? get_template_by_name($selectedBusiness['id'], $templateName) : null;
        ...
        if (!$template) {
            ...
            $response = send_legacy_template_message($cleanPhone, [
                'name'     => $template['name'],
            ...
            if (!empty($response['success'])) {
                ...
                save_outgoing_message($cleanPhone, $previewText, $wamid, 1, $selectedBusiness['id'], 'sent', null, 'template');
                ...
                save_outgoing_message($cleanPhone, $previewText, 'wamid.failed_' . time(), 1, $selectedBusiness['id'], 'failed', $errorMessage, 'template');
        ...
        if (empty($mediaUrl)) {
            ...
            $response = whatsapp_send_media($cleanPhone, $mediaType, $mediaUrl, $caption !== '' ? $caption : null, $selectedBusiness);
            ...
            if (!empty($response['success'])) {
                ...
                save_outgoing_message($cleanPhone, $previewText, $wamid, 1, $selectedBusiness['id'], 'sent', null, $mediaType, $mediaUrl, $mediaType);
                ...
                save_outgoing_message($cleanPhone, $previewText, 'wamid.failed_' . time(), 1, $selectedBusiness['id'], 'failed', $errorMessage, $mediaType, $mediaUrl, $mediaType);
        ...
        if ($interactiveType === 'list') {
            ...
            if (empty($bodyText)) {
                ...
                $response = whatsapp_send_interactive_list($cleanPhone, $headerText, $bodyText, $footerText, $buttonText, $sections, $selectedBusiness);
                ...
                if (!empty($response['success'])) {
                    ...
                    save_outgoing_message($cleanPhone, $previewText, $wamid, 1, $selectedBusiness['id'], 'sent', null, 'interactive', null, null, $interactivePayload);
                    ...
                    save_outgoing_message($cleanPhone, $previewText, 'wamid.failed_' . time(), 1, $selectedBusiness['id'], 'failed', $errorMessage, 'interactive', null, null, $interactivePayload);
            ...
            if (empty($bodyText) || empty($buttons)) {
                ...
                $response = whatsapp_send_interactive_buttons($cleanPhone, $bodyText, $buttons, $selectedBusiness);
        ...
        if (empty($message)) {
            ...
            $response = whatsapp_send_text($cleanPhone, $outgoingMessage, $selectedBusiness);

            if (!empty($response['success'])) {

                $wamid = $response['data']['messages'][0]['id'] ?? ('wamid.simulated_' . time());

                save_outgoing_message($cleanPhone, $outgoingMessage, $wamid, $allowReply, $selectedBusiness['id'], 'sent', null);
                ...
                save_outgoing_message($cleanPhone, $outgoingMessage, 'wamid.failed_' . time(), $allowReply, $selectedBusiness['id'], 'failed', $errorMessage);
...
<body class="bg-light min-vh-100 d-flex flex-column">
    ...
    <script>
        ...
        const templatesByBusiness = <?= json_encode($templatesByBusiness, JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?>;
        const seededButtons = <?= json_encode($_POST['button_label'] ?? [], JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?>;
        const seededSectionTitles = <?= json_encode($_POST['section_title'] ?? [], JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?>;
        const seededSectionRowIds = <?= json_encode($_POST['section_row_id'] ?? [], JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?>;
        const seededSectionRowTitles = <?= json_encode($_POST['section_row_title'] ?? [], JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?>;
