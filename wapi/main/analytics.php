...
$timeSeries = get_messages_over_time($days, $businessFilter);
$statuses   = get_status_breakdown();
$businesses = get_business_breakdown(8);
$types      = getTypeBreakdown();
$templatePerf = get_template_performance();
$topCustomers = get_top_customers(5);

$msgStats = get_message_stats();
$contactStats = get_customer_stats();
...
<body class="min-vh-100 d-flex flex-column">
    ...
    <script>
        ...
        new Chart(document.getElementById('volumeChart'), {
            type: 'line',
            data: {
                labels: <?= json_encode($labels, JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?>,
                datasets: [
                    { 
                        label: 'Inbound', 
                        data: <?= json_encode($inboundData, JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?>, 
                    ...
                    { 
                        label: 'Outbound', 
                        data: <?= json_encode($outboundData, JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?>, 
        ...
        new Chart(document.getElementById('businessChart'), {
            type: 'bar',
            data: {
                labels: <?= json_encode(array_map(fn($r) => mb_strimwidth($r['business_name'], 0, 18, '…'), $businesses), JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?>,
                datasets: [{
                    label: 'Messages',
                    data: <?= json_encode(array_map(fn($r) => $r['total'], $businesses), JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?>,
        ...
        new Chart(document.getElementById('typeChart'), {
            type: 'doughnut',
            data: {
                labels: <?= json_encode($typeLabels, JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?>,
                datasets: [{
                    data: <?= json_encode($typeData, JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?>,
