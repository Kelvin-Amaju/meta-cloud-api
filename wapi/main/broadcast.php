...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create') {
        ...
        if (empty($phones)) {
            ...
            if (!$result['success']) {
                ...
                foreach ($phones as $phone) {
                    if (save_campaign_recipient($campaignId, $phone)) {
                        $saved++;
                    }
                }
        ...
        $result = run_campaign($campaignId);
        if (!$result['success']) {
...
$campaigns = get_campaigns($page, $perPage);
$rows       = $campaigns['data'];
...
$recipientCounts = [];
foreach ($rows as $c) {
    $recipientCounts[$c['id']] = get_campaign_recipient_counts((int)$c['id']);
}
